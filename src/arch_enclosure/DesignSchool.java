...
public strictfp class DesignSchool extends Robot {
    ...
    @Override protected void turn() throws GameActionException {
        ...
        int reserve = built >= 8 && vaps < 2 && rc.getRoundNum() >= 250 ? C.VAPORATOR_BANK : 0;   // stage 11: after eight bodies, leave the builder its vaporator money until two stand inside (the field runs four by r700; we ran none and starved)
        boolean want = built < C.LANDSCAPERS_MAX && waiting < 2 && soup >= RobotType.LANDSCAPER.cost + reserve + (built < 16 ? 0 : C.HELPER_BANK);   // the enclosure: bodies all game
