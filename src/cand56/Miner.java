...
public strictfp class Miner extends Robot {
    ...
    private boolean build() throws GameActionException {
        ...
        if (want == RobotType.DESIGN_SCHOOL && rushSeen()) return rushSchool(home);   // Iteration 56
    ...
    private boolean rushSchool(MapLocation home) throws GameActionException {
        int sx = 0, sy = 0, n = 0;
        ...
        for (int i = 8; --i >= 0;) { Direction d = DIRS[i]; MapLocation t = loc.add(d);
            if (Nav.cheb(t, home) < 2 || !rc.canBuildRobot(RobotType.DESIGN_SCHOOL, d) || rc.senseFlooding(t)) continue;
        ...
        rc.buildRobot(RobotType.DESIGN_SCHOOL, bestD); builtSchool++; schoolLoc = loc.add(bestD); rushStand = null;
        Debug.log("@build t=" + RobotType.DESIGN_SCHOOL.ordinal() + " at=" + schoolLoc + " rush soup=" + rc.getTeamSoup());
        return true;
    }
