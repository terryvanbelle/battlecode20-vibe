...
public strictfp class Drone extends Robot {
    ...
    private boolean charging = false, carryingFriend = false; private int bareSeen = -1000;
    /** Iteration 70 (EmaPajic's finish): carry one of our landscapers to the enemy HQ and set it down beside it, where it
     *  buries the HQ (an attacker there does it unprompted). */
    private void deliver() throws GameActionException {
        MapLocation eh = MapState.enemyHQ != null ? MapState.enemyHQ : MapState.enemyHQGuess(); if (eh == null) return;
        for (int i = 8; --i >= 0;) { Direction d = DIRS[i]; MapLocation n = loc.add(d); if (Nav.cheb(n, eh) == 1 && rc.canDropUnit(d) && !rc.senseFlooding(n)) { rc.dropUnit(d); carryingFriend = false; Debug.log("@deliver at=" + n); return; } }
        nav.setTarget(eh); nav.step();
    }
    ...
    @Override protected void turn() throws GameActionException {
        ...
        if (carryingFriend && rc.isCurrentlyHoldingUnit()) { deliver(); return; }   // Iteration 70
        if (!rc.isCurrentlyHoldingUnit()) carryingFriend = false;
    ...
    private boolean raid() throws GameActionException {
        ...
        // Iteration 70: a bare ring seen lately -- fetch a body (checked before the swarm logic: a fetcher flies alone)
        if (round - bareSeen < 150) {   // a bare ring seen lately: fetch a helper (Chebyshev 2 of our HQ, never a seat) or an attacker, flying home for one
            RobotInfo f = null; int fd = 1 << 30;
            for (int i = nFriend; --i >= 0;) { RobotInfo r = friends[i]; if (r.type != RobotType.LANDSCAPER || Nav.cheb(r.location, MapState.home) <= 1 || Nav.cheb(r.location, eh) <= 1) continue; int d = loc.distanceSquaredTo(r.location); if (d < fd) { fd = d; f = r; } }
            if (f != null) { if (rc.canPickUpUnit(f.ID)) { rc.pickUpUnit(f.ID); carryingFriend = true; Debug.log("@fetch id=" + f.ID + " from=" + f.location); return true; } nav.setTarget(f.location); nav.step(); return true; }
            if (rc.getID() % 4 == 0) { nav.setTarget(MapState.home); nav.step(); return true; }   // one in four goes home for one
        }
        ...
        if (charging) {
            ...
            // Iteration 70: nothing of theirs within r2 18 of their HQ: their ring is bare -- fetch one of our landscapers
            // (away from our own HQ: an attacker or a stray, never a seat) and set it down beside their HQ
            boolean bare = true; for (int i = nEnemy; --i >= 0;) { RobotInfo e = enemies[i]; if (e.type == RobotType.LANDSCAPER && Nav.cheb(e.location, eh) <= 1) { bare = false; break; } }
            if (bare && rc.canSenseLocation(eh)) {
                bareSeen = round;
            }
