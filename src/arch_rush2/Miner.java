...
public strictfp class Miner extends Robot {
    ...
    @Override protected void init() throws GameActionException {
        ...
        // arch_rush2 (2026-09-26, PROMPTS 39: learn to rush, then learn to defend): poortho's rush as its replays show it
        // (GSF, reviewable): the FIRST miner walks to our HQ, plants a school ON our ring by r60, and its landscapers bury
        // the HQ by r130. arch_rush planted at Chebyshev 3 with its third miner and did not reproduce it.
        rusher = birth == 2 && MapState.home != null;   // birth counts from 2 for the first miner
        builder = birth == 3 && MapState.home != null;
    ...
    private void rush() throws GameActionException {
        ...
        if (loc.distanceSquaredTo(t) <= 8 && MapState.enemyHQ != null) {   // arch_rush2: beside the ring, so the school lands on it
