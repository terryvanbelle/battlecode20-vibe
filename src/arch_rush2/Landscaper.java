...
public strictfp class Landscaper extends Robot {
    ...
    @Override protected void init() throws GameActionException {
        super.init();
        RobotInfo[] en = rc.senseNearbyRobots(-1, us.opponent());   // arch_rush: born beside the enemy HQ, bury it
        for (int i = en.length; --i >= 0;) if (en[i].type == RobotType.HQ) MapState.enemyHQ = en[i].location;
        if (MapState.home == null) { attacker = true; Debug.log("@rush attacker"); }   // born at the forward school
    }
    ...
    private void turn2() throws GameActionException {
        ...
        if (attacker && home == null) { attack(); return; }   // arch_rush: the forward landscapers have no home in sight
    ...
    private void attack() throws GameActionException {
        ...
        if (g == null && MapState.originKnown()) g = MapState.center();
        if (g == null) return;   // arch_rush: a forward landscaper with nothing in sight holds
