...
public strictfp class Miner extends Robot {
    ...
    private boolean highBuild() throws GameActionException {
        if (loc.equals(hsStand) || loc.isAdjacentTo(hsTarget) && Math.abs(rc.senseElevation(loc) - rc.senseElevation(hsTarget)) <= 3) {
            ...
            if (nBadHigh < badHigh.length) badHigh[nBadHigh++] = hsTarget; hsTarget = null; return false;
        }
        nav.setTarget(hsStand); if (nav.stalled()) { if (nBadHigh < badHigh.length) badHigh[nBadHigh++] = hsTarget; hsTarget = null; buildPause = round + 20; return false; }
    ...
    private boolean build() throws GameActionException {
        ...
        else if (builtSchool > 0 && builtHigh < C.HIGH_VAP_MAX && round >= C.HIGH_VAP_FROM && soup >= C.HIGH_VAP_BANK && highSite(home)) return highBuild();   // Iteration 83
