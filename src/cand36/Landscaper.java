...
public strictfp class Landscaper extends Robot {
    ...
    @Override protected void init() throws GameActionException {
        super.init();
        for (int r = Math.max(1, round - 25); r < round && MapState.site == null; r++) {   // Iteration 36: learn the site before picking a role
    ...
    private void turn2() throws GameActionException {
        ...
        if (helper && round < 700 && round % 15 == id % 15 && pickMason(home)) { helper = false; post = null; mason = true; Debug.log("@mason site=" + MapState.site); }
    ...
    private boolean pickMason(MapLocation home) throws GameActionException {
        ...
        if (rc.canSenseLocation(st)) { RobotInfo r = rc.senseRobotAtLocation(st); if (r != null && r.ID != id && r.type == RobotType.LANDSCAPER && r.team == us) return false; }
