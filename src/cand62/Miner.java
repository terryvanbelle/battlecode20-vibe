...
public strictfp class Miner extends Robot {
    ...
    private boolean build() throws GameActionException {
        ...
        // Iteration 62: the school where we stand, now -- under a rush in sight, or once r90 has come with the bank for it.
        // arch_rush2 (poortho's rush, PROMPTS 39) on GSF: the builder's walk to its stand stalled at r75, the stall paused
        // all building for 50 rounds, and the school came at r162 with 580 soup banked; the HQ was buried by r166.
        if (builtSchool == 0 && rc.getTeamSoup() >= RobotType.DESIGN_SCHOOL.cost && (rushSeen() || round >= 90) && rc.isReady() && Nav.cheb(loc, home) <= 3) {   // only near home (the first form built one ten out)
            Direction bd = null; int bs = Integer.MIN_VALUE;
            for (int i = 8; --i >= 0;) { Direction d = DIRS[i]; MapLocation t = loc.add(d); if (Nav.cheb(t, home) < 2 || !rc.canBuildRobot(RobotType.DESIGN_SCHOOL, d) || rc.senseFlooding(t)) continue;
                int sc = -Nav.cheb(t, home) * 10 + (Nav.cheb(t, home) <= 3 ? 100 : 0); if (sc > bs) { bs = sc; bd = d; } }
            if (bd != null) { rc.buildRobot(RobotType.DESIGN_SCHOOL, bd); builtSchool++; Debug.log("@build t=4 at=" + loc.add(bd) + " now soup=" + rc.getTeamSoup()); return true; }
        }
        if (builtSchool == 0 && (rushSeen() || round >= 90) && Nav.cheb(loc, home) > 3) { nav.setTarget(home); nav.step(); return true; }   // Iteration 62: come home to build it
