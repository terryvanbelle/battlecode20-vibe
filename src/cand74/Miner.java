...
public strictfp class Miner extends Robot {
    private boolean builder, rusher, planted; private int nearRounds = 0; private int probeRounds = 0, probeSign = -1;
    ...
    private void rush() throws GameActionException {
        ...
        // Iteration 74: a rusher in sight of the enemy HQ that cannot get within r2 8 (cliffs, water, their wall) for 15 rounds
        // plants where it stands, if still within r2 25 (the rush census: 47 of 104 never planted)
        if (MapState.enemyHQ != null && loc.distanceSquaredTo(t) <= 25 && loc.distanceSquaredTo(t) > 8) { if (++nearRounds >= 15 && rc.getTeamSoup() >= RobotType.DESIGN_SCHOOL.cost && tryBuild(RobotType.DESIGN_SCHOOL, t)) { planted = true; Debug.log("@rushplant far at=" + loc + " hq=" + t); return; } }
