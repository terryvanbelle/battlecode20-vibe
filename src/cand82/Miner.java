...
public strictfp class Miner extends Robot {
    private boolean builder, rusher, planted; private int plantPosts = 0; private int probeRounds = 0, probeSign = -1;
    ...
    private void turn2() throws GameActionException {
        ...
        if (rusher && planted && plantPosts < 3 && post(Comms.make(Comms.RUSH_PLANTED, round, us, MapState.rushPlantRound))) plantPosts++;   // Iteration 82: three rounds running (a miner reads one block in three)
    ...
    private boolean build() throws GameActionException {
        ...
        if (burstHold()) return false;   // Iteration 82: the bank is the burst's until our school is planted
