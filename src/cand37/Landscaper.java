...
public strictfp class Landscaper extends Robot {
    ...
    private void work(MapLocation home) throws GameActionException {
        ...
        if (es >= e0 + C.SITE_RAISE && eo >= e0 + C.SITE_STAND) { mason = false; Debug.log("@mason done site=" + s); return; }
        ...
        if (bestD != null) { rc.digDirt(bestD); digs++; masonIdle = 0; } else if (++masonIdle > 20) { mason = false; Debug.log("@mason no dirt"); }   // 37: nothing to dig from (water all round the stand on Squares): a helper instead
    }
    private int masonIdle = 0;
    ...
    private void attack() throws GameActionException {
        ...
        if (rc.getDirtCarrying() < RobotType.LANDSCAPER.dirtLimit && rc.isReady() && Nav.cheb(loc, MapState.home) > 2) {
            for (int i = 8; --i >= 0;) { Direction dd = DIRS[i]; MapLocation n = loc.add(dd); if (!rc.canDigDirt(dd) || onRing(n)) continue; RobotInfo r = rc.canSenseLocation(n) ? rc.senseRobotAtLocation(n) : null; if (r != null) continue; rc.digDirt(dd); digs++; return; }   // 36b: never our ring or the posts beside the school
        }
