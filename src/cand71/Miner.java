...
public strictfp class Miner extends Robot {
    ...
    private boolean enemyMinerNearHome() { MapLocation h = MapState.home; if (h == null) return false; for (int i = nEnemy; --i >= 0;) if (enemies[i].type == RobotType.MINER && enemies[i].location.distanceSquaredTo(h) <= 36) return true; return false; }
    // ---------------------------------------------------------------- builder
    private boolean build() throws GameActionException {
        ...
        // Iteration 71: the rusher's miner itself is the alarm -- an enemy miner within 6 of our HQ before r120 means a school
        // beside our ring within twenty rounds (g_iter12's rusher plants at r49); a center now puts a drone over it first
        else if (builtFC == 0 && round < 120 && enemyMinerNearHome() && soup >= RobotType.FULFILLMENT_CENTER.cost) { want = RobotType.FULFILLMENT_CENTER; Debug.log("@alarm center"); }
        ...
        if (want == RobotType.FULFILLMENT_CENTER && (rushSeen() || (round < 120 && enemyMinerNearHome()))) return rushBuild(want, home);   // Iteration 57: the rush center where the rusher is not (Iteration 54's never found a site)
