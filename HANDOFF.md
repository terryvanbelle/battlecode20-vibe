...
- **Running on the VM:** `gate33` (Iteration 33, `src/bot` = `src/cand33`: a miner deposits only a load of 70 or more, or
  what it carries once the soup within reach is gone; mirror vs g_iter6, seeded). Iterations 31 (gun perches; 7-25,
  13-35) and 32 (miners off the circle; 77-83) rejected today, code in `src/cand31*`, `src/cand32`. No incumbent blocks
  (PROMPTS 26). Diagnostics run on the VM when it is idle (`tools/vm-run.sh diagN '... tools/run-dev.sh ...'`).
