...
- **VM idle. The enclosure program is CLOSED** (TRAINING_LOG "THE ENCLOSURE PROGRAM IS CLOSED"): stage 36 (interior feeders) added +60 of shell at r3000; with every hole closed the shell stands 715 below g_iter9's wall and g_iter9 outlives it on all three maps. `src/arch_enclosure` is kept as a reference and sparring archetype. The loop returns to the incumbent g_iter9. **Running: `census9`** -- g_iter9 against itself on all 52 maps (seed 7), bodies and soup at r300/500/800 per side and the result, to find maps where the incumbent's own production fails (the old GSF/Climb/Hills note: 0-3 landscapers by r500).
