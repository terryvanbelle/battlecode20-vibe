...
- **Running on the VM:** block 46 (`tools/scrim.sh`, BOT=g_iter5, N=48, the band) and `gate28c` (Iteration 28c in
  `src/bot`: refinery and school outward of the Chebyshev-2 circle, nearest stand; mirror vs g_iter5). Iteration 28 was
  rejected 24-40; 28b is kept provisionally (129-111 over 240, commit 79fd4a5). Post blocks with `tools/post-block.sh <run> g_iter5`, then
...
- **Open lines, in order:** (1) map-dead games: the idle miners were sealed behind the HQ by our own refinery
  (Iteration 28 rejected 24-40; 28b provisional at 53.8%; 28c gating). Next on Climb: six landscapers cannot reach the west seats (`@badseat`) because digging
