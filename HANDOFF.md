...
- **The lattice program (`src/cand78`; TRAINING_LOG "The lattice"), stages 1-10:** the income appears (up to 25,000 soup), but the buildings drown -- too few bodies to hold ~60 grid tiles. Checkpoint at stage 10; next idea: vaporators on safe ground first (inside the ring or on high tiles). `src/bot` = g_iter12. Origin: A raised grid (one parity of rows
...
- **Incumbent: `src/g_iter12`** (g_iter11 + our own poortho-style rush, Iteration 63; gate 63 ACCEPT 60-34 in the
  paired mirror). **Submitted: 336 games, 159-177, 1818 +- 39, rank 13 of 88** (+79 on g_iter11 at 240; 53 wins before r1300 vs 18). Gate 65 (`src/cand65` = g_iter12 + the late drone raid): 5-0, provisional. Arm 65 (96 games): 1789 +- 73, null; the raid line closed. Gate 66 (the miner freeze vs g_iter12's rush): 10-14, not kept; diagnostic 67 (first-miner rusher) refuted. Gate 68 (14 forward landscapers): 0-3, not kept. Raid at scale (`cand69`): arm69 1811 +- 72 = g_iter12, null; raid line closed. Gate 72 (poortho's vaporator economy): REJECT 3-20. `src/bot` = g_iter12. Rush census read (15 of 57 plants spawned nothing: the home school took the bank). **Running: `gate73`** (`src/cand73`, `src/bot` = cand73: the home school yields to the forward school). g_iter11: 240 games, 1751 +- 47, rank 13.
...
- **VM idle. The enclosure program is CLOSED** (TRAINING_LOG "THE ENCLOSURE PROGRAM IS CLOSED"): stage 36 (interior feeders) added +60 of shell at r3000; with every hole closed the shell stands 715 below g_iter9's wall and g_iter9 outlives it on all three maps. `src/arch_enclosure` is kept as a reference and sparring archetype. The loop returns to the incumbent g_iter9. **`census9` read** (TRAINING_LOG "The production census"): ten starved maps (Spiral, GSF, Hills, Climb, DidAMonkeyMakeThis, AMaze, CosmicBackgroundRadiation, Islands, Maze, TheHighGround) where g_iter9 has 3-9 landscapers at r500 and mines 130-400 by r300. `diagecon` read: every miner freezes by r300 on Spiral and GSF (a soup target alternating with the explore target resets the navigator's stall counter every turn). **Iteration 50 (`src/cand50`, `src/bot` = cand50): the miner never falls through to explore while a soup target stands.** Diagnostic read (mining after r300 3-6x on Spiral; the mechanism fires). **ACCEPTED: gate50 32-9 discordant at 112 pairs -> `src/g_iter10`** (`src/bot` = g_iter10). Submission blocks `sub10` played and posted. Iteration 51 (the school needs three doors) NOT KEPT: gate51 5-3 discordant in 320 pairs. `src/bot` = g_iter10. `census10` read (TRAINING_LOG): the fix lifted Swirl, WateredDown, Egg, SoupOnTheSide, Spiral; still starved Hills, GSF, TheHighGround, Islands, AMaze, Maze, Climb A. `diaghills` read: miners random-walk near the base with the origin unknown and never find far soup. Iteration 52 (rays) NOT KEPT: gate52 52-53 in 320 pairs. `src/bot` = g_iter10. `seats10` clean (nothing of ours blocks the wall). Iteration 53 (the rush answer: bury the enemy school, no miners during a rush) REFUTED at the diagnostic vs arch_rush (died sooner: the school is rebuilt, the HQ goes unguarded). `src/bot` = g_iter10. A reproducible harness for the rush: g_iter10 as A vs `arch_rush` on InADitch, seed 7, dies at r292 (TRAINING_LOG). Iteration 54 (early center and drones under a rush) REFUTED at the diagnostic: the center never found a site, the school stayed shut with soup idle. `src/bot` = g_iter10. The rush harness read: our school is buried at r164 and never rebuilt. **Iteration 55 (`src/cand55`, `src/bot` = cand55): rebuild a lost school (detected by its tile), dig our school out.** Diagnostic: InADitch A r292 -> r704, quiet game unchanged. Gate 55 (mirror) 1-0 and gate 55r (vs arch_rush) 7-7: not kept. **g_iter10 loses about half its games to our own `arch_rush`** (its school is buried beside the HQ and never rebuilt). Iteration 56 (the far school) refuted (CowFarm r949 -> r138). **Iteration 57 (`src/cand57`, `src/bot` = cand57): under a rush, a fulfillment center placed on the far side and three drones at cost. Diagnostic: fires, mixed (MtDoom r163 -> r957, CowFarm r949 -> r421). Gate57r 21-13 (p 0.23), not kept.** **Iteration 58 (`src/cand58`, `src/bot` = cand58): the rush center buys three drones at cost even out of the rusher's sight. Diagnostic: MtDoom r163 -> r2934, FourLakeLand r678 -> r2933, CowFarm r949 -> r421. Gate 58r ACCEPT 28-6, gate 58 (mirror) 2-2 -> **g_iter11**.**
