...
- **Running on the VM now: `diagenc35`** -- the enclosure's stage-35 diagnostic (stage 34 fixed the gate, but stray miners on the west shell and a lift loop opened new holes, and stage 33's drone gate-ban starved Squares; stage 35 addresses all three). Read it with `source tools/vm.sh; ensure_vm; gssh "cat ~/projects/vibe/2020/gauntlet/diagenc35.log"` (ends with DIAGDONE). Each stage's command is the same shape (TRAINING_LOG "Stage 22" onward); `tools/enc-read.py` summarises a game, `tools/enc-acct.py <log> <side> <hqx> <hqy>` gives the holders' dirt accounting by ring distance; `--ring 100 --ringd 2` lists the 16 shell tiles in order dx=-2..2 outer, dy=-2..2 inner (index 13 = dx 2, dy 0).
