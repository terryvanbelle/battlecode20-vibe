...
gssh "mkdir -p ~/$REMOTE_REPO ~/projects/vibe/bc20-benchmarks ~/jdk"
if ! gssh "test -x ~/jdk/jdk8u504-b01/bin/java"; then
  echo "pushing JDK 8 ..."; tar -C "$HOME/jdk" -czf - jdk8u504-b01 | gssh "tar -C ~/jdk -xzf -"
fi
if [ "${SKIP_BENCH:-0}" != 1 ] && { [ "${FULL:-0}" = 1 ] || ! gssh "test -f ~/projects/vibe/bc20-benchmarks/manifest.tsv"; }; then
  echo "pushing benchmark classes + manifest ..."
  tar -C "$HOME/projects/vibe/bc20-benchmarks" -czf - _classes manifest.tsv | gssh "tar -C ~/projects/vibe/bc20-benchmarks -xzf -"
fi
