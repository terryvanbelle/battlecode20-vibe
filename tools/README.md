...
| `mirror.sh` (BOT, REF, N, BATCH, W0/L0, PAIRED) | the accept gate: random map, side and seed per cell, batches of 16; `PAIRED=1` (default) plays each cell as the candidate vs the incumbent and the incumbent vs itself (`paired.sh`), N counts pairs and `sprt.py` runs on the discordant pairs only; `PAIRED=0` is the old unpaired mirror |
| `field-score.py [--plot --tau]` | `progress/field-score.png`: every build's field score over time; the submissions' ratings fitted with a diminishing-returns curve (R0 + a ln(1 + t/tau)) and projected 7 and 14 days out with a 95% band, mapped to field scores through the current ladder fit; run by `post-block.sh` after every block (PROMPTS 28-30) |
