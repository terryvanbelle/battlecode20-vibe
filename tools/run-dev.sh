...
#   tools/run-dev.sh <teamA> <teamB> <map> [replay.bc20] [extra -D flags...]
#   LOG_OUT=game.log tools/run-dev.sh bot x Map r.bc20 -Dbc.server.robot-player-to-system-out=true   # keep the engine stdout (bot @tag lines)
set -euo pipefail
...
NA="$1"; NB="$2"; MAP="$3"; REPLAY="${4:-$REPO/matches/$NA-vs-$NB-on-$MAP.bc20}"; shift 3; [ $# -gt 0 ] && shift
declare -A URL_OF
