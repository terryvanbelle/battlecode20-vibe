...
#
#   tools/run-match.sh <teamA> <teamB> <map> [replay.bc20] [extra -D flags...]
#
...
# and leaves the replay where asked (default matches/<A>-vs-<B>-on-<map>.bc20).
set -euo pipefail
REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "$REPO/tools/lib.sh"
TA="$1"; TB="$2"; MAP="$3"; REPLAY="${4:-$REPO/matches/$TA-vs-$TB-on-$MAP.bc20}"; shift 3; [ $# -gt 0 ] && shift
